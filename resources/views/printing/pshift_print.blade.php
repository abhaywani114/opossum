...
function escpos_print_template() {
	...
	'\x0A',
    '\x1B' + '\x21' + '\x08',	// emphasized mode on
    'Personal Shift    '+ '{{$login_time}} {{$logout_time}}'+ '\x0A',
	'---------------------------------------------' + '\x0A',
...
//   '123456789012345678901234567890123456789012345'
	'\x1B' + '\x21' + '\x00',	// emphasized mode off
	'Today Sales                        '+ '{{str_pad( $sales,10, " ", STR_PAD_LEFT)}}' + '\x0A',

    '{{!empty($terminal->taxtype)?strtoupper($terminal->taxtype):"SST"}} {{ $terminal->tax_percent??"6"}}{{str_pad("%",20," ",STR_PAD_RIGHT)}}{{str_pad($tax, 20, " ", STR_PAD_LEFT)}}' + '\x0A',
...
//    '123456789012345678901234567890123456789012345'
	...
	'Cash                               '+ '{{str_pad($cash, 10, " ", STR_PAD_LEFT)}}' + '\x0A',

    'Credit Card                        ' +'{{str_pad($creditcard, 10, " ", STR_PAD_LEFT)}}' + '\x0A',


	'Wallet                        ' +'{{str_pad($wallet, 10, " ", STR_PAD_LEFT)}}' + '\x0A',


    'Outdoor Payment Terminal ' +'{{str_pad($creditcard, 20, " ", STR_PAD_LEFT)}}' + '\x0A',
