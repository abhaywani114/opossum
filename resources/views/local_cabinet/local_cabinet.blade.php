			...
			@foreach ($receipt as $r)
				<tr id="cloningrow">
					<td style="text-align: center">
						<p style="margin:0;">{{$loop->index + 1}}</p>
					</td>
					<td class="text-center">
						<a href="#" onclick="eod_summarylist('{{$r->day}}')"
					...
					<td class="text-center">
						<a href="{{ route('fuel-receipt-list',  ['date'=> date('Y-m-d', $r->created_at)]) }}" target="_blank"
						   style="text-decoration: none;">
						   {{$r->count_receipt_fuel}}
						</a>
					</td>
					<td class="text-center">
						<a href="{{route('local_cabinet.cstore_receipt_landing', $r->day)}}"
							target="_blank" xonclick="receipt_list('{{$r->day}}')"
						   style="text-decoration: none;">
							{{$r->count_receipt_cstore}}
						</a>
					</td>
				</tr>
			@endforeach
