...
<div class="row">
	<div class="col-md-12 rate_mg d-flex">
		<div id="inclusive_rate" onclick="select_rate_type('inclusive')"
			...
			<?php if(isset($terminal->mode)): ?>
				<?php if($terminal->mode == 'inclusive'): ?>
					color:#34dabb;
				<?php else: ?>
					cursor: pointer;
				<?php endif; ?>
			<?php endif; ?>">Inclusive</div> |
		...
		<div id="exclusive_rate" onclick="select_rate_type('exclusive')"
			...
			style="width:80px;font-weight:bold;
			<?php if(isset($terminal->mode)): ?>
				<?php if($terminal->mode == 'exclusive'): ?>
					color:#34dabb;
				<?php else: ?>
					cursor: pointer;
				<?php endif; ?>
			<?php endif; ?>">Exclusive</div>
		-->

		<p id="select_rate_type" style="display:none">
            <?php echo e($terminal->mode??""); ?>

            <input type="hidden" id="old_mode"  value="<?php echo e($terminal->mode??""); ?>">
            <input type="hidden" id="old_taxtype"  value="<?php echo e($terminal->taxtype??""); ?>">
		</p>
	</div>
	...
	<div id="idofSST" class="rateval" onclick="select_text_type('sst')"
			...
			<?php if(isset($terminal->taxtype)): ?>
				<?php if($terminal->taxtype == 'sst'): ?>
					color:#34dabb;
				<?php else: ?>
					cursor: pointer;
				<?php endif; ?>
        ...
        <div style="float:right;text-align: center;">
            <input type="hidden" id="old_sst_gst_vat_field"  value="<?php echo e(number_format($terminal->tax_percent,2)); ?>">
			<input type="text" id="sst_gst_vat_field"
			   name="" size="7"
			   class="text-center"
			   style="width:80px;border-radius:5px;"
			   value="<?php echo e(number_format($terminal->tax_percent,2)); ?>"> %
...
<?php /**PATH D:\Server_Files\oceania\trunk\oceania\resources\views/screen_d/rate.blade.php ENDPATH**/ ?>